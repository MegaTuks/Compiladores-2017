...
goog.provide('Blockly.tuby.texts');

goog.require('Blockly.tuby');


Blockly.tuby['text'] = function(block) {
  var text = block.getFieldValue('TEXT'); 
  // Text value.
  var code = '"' + text + '"';
  // Blockly.tuby.quo(block.getFieldValue('TEXT'));
  return [code, Blockly.tuby.ORDER_ATOMIC];
};

Blockly.tuby['text_join'] = function(block) {
  // Create a string made up of any number of elements of any type.
  var code;
  if (block.itemCount_ == 0) {
    return ['\'\'', Blockly.tuby.ORDER_ATOMIC];
  } else if (block.itemCount_ == 1) {
    var argument0 = Blockly.tuby.valueToCode(block, 'ADD0',
    ...
    var argument0 = Blockly.tuby.valueToCode(block, 'ADD0',
        Blockly.tuby.ORDER_NONE) || '\'\'';
    var argument1 = Blockly.tuby.valueToCode(block, 'ADD1',
        Blockly.tuby.ORDER_NONE) || '\'\'';
    code = 'String(' + argument0 + ') + String(' + argument1 + ')';
    return [code, Blockly.tuby.ORDER_ADDITION];
  } else {
    code = new Array(block.itemCount_);
    for (var n = 0; n < block.itemCount_; n++) {
      code[n] = Blockly.tuby.valueToCode(block, 'ADD' + n,
          Blockly.tuby.ORDER_COMMA) || '\'\'';
    }
    code = '[' + code.join(',') + '].join(\'\')';
    return [code, Blockly.tuby.ORDER_FUNCTION_CALL];
  }
...
Blockly.tuby['text_append'] = function(block) {
  // Append to a variable in place.
  var varName = Blockly.tuby.variableDB_.getName(
      block.getFieldValue('VAR'), Blockly.Variables.NAME_TYPE);
  var argument0 = Blockly.tuby.valueToCode(block, 'TEXT',
      Blockly.tuby.ORDER_NONE) || '\'\'';
  return varName + ' = String(' + varName + ') + String(' + argument0 + ');\n';
};

Blockly.tuby['text_length'] = function(block) {
...
Blockly.tuby['text_isEmpty'] = function(block) {
...
Blockly.tuby['text_indexOf'] = function(block) {
...
Blockly.tuby['text_getSubstring'] = function(block) {
  // Get substring.
  var text = Blockly.tuby.valueToCode(block, 'STRING',
      Blockly.tuby.ORDER_MEMBER) || '\'\'';
...
Blockly.tuby['text_changeCase'] = function(block) {
  ...
  var code;
  if (operator) {
    // Upper and lower case are functions built into tuby.
    var argument0 = Blockly.tuby.valueToCode(block, 'TEXT',
        Blockly.tuby.ORDER_MEMBER) || '\'\'';
    code = argument0 + operator;
  } else {
    // Title case is not a native tuby function.  Define one.
    var functionName = Blockly.tuby.provideFunction_(
        'text_toTitleCase',
        [ 'function ' +
            Blockly.tuby.FUNCTION_NAME_PLACEHOLDER_ + '(str) {',
...
Blockly.tuby['text_trim'] = function(block) {
  // Trim spaces.
  var OPERATORS = {
    'LEFT': ".replace(/^[\\s\\xa0]+/, '')",
    'RIGHT': ".replace(/[\\s\\xa0]+$/, '')",
  ...
  var argument0 = Blockly.tuby.valueToCode(block, 'TEXT',
      Blockly.tuby.ORDER_MEMBER) || '\'\'';
  return [argument0 + operator, Blockly.tuby.ORDER_FUNCTION_CALL];
};
...
Blockly.tuby['text_prompt_ext'] = function(block) {
  // Prompt function.
  if (block.getField('TEXT')) {
    // Internal message.
    var msg = Blockly.tuby.quote_(block.getFieldValue('TEXT'));
  } else {
    // External message.
    var msg = Blockly.tuby.valueToCode(block, 'TEXT',
        Blockly.tuby.ORDER_NONE) || '\'\'';
  }
...
Blockly.tuby['text_prompt'] = Blockly.tuby['text_prompt_ext'];
